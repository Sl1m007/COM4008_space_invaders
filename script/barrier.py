...
# Each barrier is made up of many small blocks.
# When a bullet hits a block, only that piece is removed, which makes
# the barriers slowly crumble as the game continues.
#############################################################################
...
class BarrierBlock(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, size=6):
        ...
        # Draw a tiny green square to represent one cell of the barrier
...
############################################################
# Tiny 2D matrix used to build a barrier.
# Each row represents a row of cells.
# A "#" means a solid barrier cell.
...
def create_barrier(barrier_group, all_sprites, start_x, start_y, block_size=6):
    ##########################################################################
    # Build a single barrier using the tiny 2D matrix above.
    # Each "#" cell is converted into a BarrierBlock sprite.
    ##########################################################################

    for row_index, row in enumerate(BARRIER_MATRIX):
...
def create_barriers(barrier_group, all_sprites, block_size=6):
    #################################################################
    ...
    # Create four barriers evenly spaced across the screen
    for i in range(4):
        start_x = 100 + i * spacing
        create_barrier(barrier_group, all_sprites, start_x, base_y, block_size)